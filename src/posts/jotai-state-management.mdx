...
<Box ml="2em">
  <ul>
    ...
    <li>
      {" "}
      When the parent component re-renders, the child component re-renders as
      well{" "}
    </li>
  </ul>
</Box>
...
contrast to Redux and Zustand, which use a huge state that consist of lots of
smaller states as its attribute.
...
<Box sx={{ marginLeft: "2em" }}>
  <ul>
    <li>
      {" "}
      useAtomValue() — When we need to access atom’s value only in a
      component{" "}
    </li>
    <li> useSetAtom() — When we need to update atom’s value in a component </li>
    <li>
      {" "}
      useAtom() — When we need both access and updating atom’s value in a
      component{" "}
    </li>
  </ul>
</Box>
...
solves the extra re-render issue of React context, and eliminates the need for
any memoization.
<br />
